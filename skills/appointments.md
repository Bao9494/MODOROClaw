...
**Push target Telegram:** Khi CEO muốn nhắc/gửi vào group Telegram, tra trước `GET /api/telegram/conversations?name=<tên>&autoMode=1&enabled=true`, rồi lưu `pushTargets:[{"channel":"telegram","toId":"<targetChatId>","toName":"<label>","atTime":"HH:MM","daily":false,"template":"..."}]`. KHÔNG đoán ID từ tên.

**Push target Zalo:** Chỉ khi CEO nói rõ Zalo. Đọc `~/.openzca/profiles/default/groups.json`. Normalize accent-insensitive. 1 match → auto-fill + confirm CEO. Nhiều → list 2-3, chờ chọn. 0 → báo không tìm thấy, KHÔNG đoán groupId.
...
1. Khách xin đặt → escalate, KHÔNG tự ghi. 2. id `apt_<ms>_<rand>`. 3. `+07:00` bắt buộc — sai → fire nhầm. 4. Telegram group phải resolve bằng API Telegram; Zalo group phải resolve bằng cache Zalo, KHÔNG đoán groupId. 5. Confirm sau ghi. 6. KHÔNG touch `reminderFiredAt/pushedAt/status`. 7. KHÔNG nhắc file/JSON trong reply CEO.
