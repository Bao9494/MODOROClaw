...
function ensureTelegramReminderFastPathPatch(vendorDir, homeDir) {
  ...
  const helperLines = [
    ...
    '\tlet query = match && match[1] ? match[1].replace(/^[:\\-\\s]+/, "").replace(/[.,;!?]+$/, "").trim() : "";',
    '\tif (!query || query.length < 2) return null;',
    '\tlet contentPrefix = ""; // 20260712-telegram-reminder-target-content-v1',
    '\tconst queryExtraMatch = query.match(/^(.+?\\b(?:[A-Za-z]{2,}[-_]\\d[\\w-]*|\\d{5,})\\b)\\s+(.+)$/);',
    '\tif (queryExtraMatch && queryExtraMatch[2]) {',
    '\t\tquery = queryExtraMatch[1].trim();',
    '\t\tcontentPrefix = queryExtraMatch[2].trim();',
    '\t}',
    ...
    '\tif (contentPrefix) content = (contentPrefix + " " + content).trim();',
    '\tcontent = content.replace(/^(?:h\\u00f4m nay|hom nay|ng\\u00e0y mai|ngay mai|mai)\\s+/i, "").trim();',
    '\tcontent = content.replace(/^(?:v\\u00e0o|vao|l\\u00fac|luc|at)?\\s*\\d{1,2}(?:\\s*(?:h|:|g)\\s*\\d{1,2})?\\s*/i, "").trim();',
