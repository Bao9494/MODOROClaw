...
| Memory | `memory/zalo-users`, `memory/zalo-groups` | `memory/telegram-chats` | Chưa tách rõ user/group/chat/customer tier |
| UI | 2 cột Nhóm/Bạn bè, mode, nội bộ, bật/tắt, bulk action | 1 danh sách Telegram chung | Chưa đủ linh hoạt như Zalo |
...
- Thêm context builder chuẩn cho Telegram.
- Bind session theo `telegram:<chatId>` hoặc `telegram:<chatId>:thread:<topicId>`.
- Đưa sender role và chat role vào prompt.
...
- Chia 2 cột Group/Channel và Private/CEO/DM.
- Thêm tab/filter/bulk action/mode control.
- Mỗi dòng hiển thị role, responseMode, enabled, alias, memory count.
- Nút profile mở đúng tầng memory.
- Nút scan sâu/seed/import export rõ hơn.
...
- Thêm `memory/telegram-users`.
- Thêm `memory/telegram-groups`.
- Giữ `memory/telegram-chats` để tương thích.
- Lưu `telegram-history/<chatId>.jsonl`.
- Tạo summary profile định kỳ từ history.
...
Không sửa UI ngay. Bước tiếp theo nên là Phase 1:

1. Tách `telegram-policy.js`.
2. Chuẩn hóa schema role/mode/toolScope/memoryScope.
3. Thêm contract test để khóa hành vi.
4. Sau khi policy ổn mới đổi UI và inbound context.
