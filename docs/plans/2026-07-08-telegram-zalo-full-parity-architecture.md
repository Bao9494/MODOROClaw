...
1. Hook provider Telegram để tự lưu inbound/outbound message refs, session binding và rich directory khi có tin thật.
2. Tách memory vật lý thành `memory/telegram-users`, `memory/telegram-groups` và giữ `memory/telegram-chats` để tương thích.
