...
| History/archive | Pending history, message refs, archive theo Zalo | Telegram có `telegram-history/<chatId>.jsonl` và seed profile từ history gần đây | Còn thiếu topic/member cache chi tiết từ provider |
| Memory | `memory/zalo-users`, `memory/zalo-groups` | `memory/telegram-chats`, foundation `memory/telegram-users`, `memory/telegram-groups`, auto-fill từ history archive | Còn thiếu summary định kỳ ngoài thao tác seed/scan |
...
- Cho directory scanner đọc `memory/telegram-users` và `memory/telegram-groups` như nguồn lookup độc lập. (done foundation)
- Giữ `memory/telegram-chats` để tương thích. (done từ các phase trước)
- Lưu `telegram-history/<chatId>.jsonl`. (done foundation)
- Tạo summary profile từ history khi seed/scan, gồm người tham gia và topic/thread gần đây. (done foundation; định kỳ tự động vẫn pending)
...
- Câu hỏi phân loại Telegram theo tên/alias như "LLK-999999 là nội bộ hay khách hàng" có fast-path runtime, không phụ thuộc vào LLM/tool loop dài.
...
- 2026-07-08: Đã bổ sung Phase 5b scanner: `telegram-memory.js` đọc thêm `memory/telegram-users` và `memory/telegram-groups`, contract guard xác nhận lookup được profile chỉ tồn tại ở tier group. Chưa build/cài runtime.
- 2026-07-09: Đã bổ sung Phase 5c history archive: `electron/lib/telegram-history-archive.js`, hook vào runtime capture, bảo vệ `telegram-history` trong sacred-data, seed workspace và contract guard chống trùng `messageId`. Chưa build/cài runtime.
- 2026-07-09: Đã bổ sung Phase 5d rich scan từ history archive: `telegram-memory.js` đọc `telegram-history/<chatId>.jsonl` khi seed, tự tạo summary, `msgCount`, `lastSeen`, người tham gia gần đây và topic/thread gần đây để auto-fill hồ sơ Telegram. Guard mới xác nhận history-only chat vẫn được seed thành profile giàu.
- 2026-07-09: `npm.cmd run guard:architecture` PASS và local unsigned build PASS. Artifact: `O:\project\9bizclaw\artifacts\9BizClaw Setup 2.4.23-telegram-full-parity-unsigned-20260709.exe`, SHA256 `9A1E3722CEFAA2CA9D9A8DA650ADF61E765443C85121EFD1816EF78744776A83`. Chưa cài đè live app vì app đang chạy 4 tiến trình 9BizClaw; cần restart/cài trong cửa sổ kiểm soát.
- 2026-07-09: Runtime test phát hiện bot trả lời role LLK-999999 đúng nhưng chậm khoảng 46s vì LLM đi vòng qua tool/shell. Đã thêm contract guard và source fix: `20260709-fast-telegram-role-lookup-v1` trong `vendor-patches.js`, cron resolver đọc `X-Source-Channel: telegram` cho lookup theo `name`, và test harness bắt fail thật trước khi skip `better-sqlite3`.
- 2026-07-09: Build/cài runtime fast-role PASS. Artifact: `O:\project\9bizclaw\artifacts\9BizClaw Setup 2.4.23-telegram-fast-role-unsigned-20260709.exe`, SHA256 `2ABEC8DE453DE8FFB54EDF59C39AC061610850C014F8BF907CC91D75B0D54558`. Backup bản cài trước khi copy: `C:\Users\bao.nguyen\AppData\Local\Programs\9bizclaw-backup-20260709-telegram-fast-role`. Runtime log có `telegram-fast-role-lookup: applied`, 4 cổng gateway/router/knowledge/cron API đều listen, profile LLK trả `role=internal/toolScope=internal`, cron `name=LLK` + header Telegram tạo đúng target `-1003857797941` và cron smoke đã xóa.
